...
inline double my_log(double val){ double res = log(val); return (lessThanZero(res) || isnan(res) || isinf(res)) ? DEFAULTVALUE : res;}
inline double my_log_avoid_zero(double val){ double res = log(val); return (isnan(res) || isinf(res)) ? DEFAULTVALUE : res;}
inline double my_div(double x, double y) {return ( isZero(y) ) ? DEFAULTVALUE : x/y;}
...
inline double my_sub(double val, double val2){ double res = val - val2; return lessThanZero(res) ? DEFAULTVALUE : res;  }
