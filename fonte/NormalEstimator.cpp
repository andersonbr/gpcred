...
double NormalEstimator::MAXLOG =  7.09782712893383996732E2;
double NormalEstimator::SQRTH =  7.07106781186547524401E-1;

double NormalEstimator::round(double data) {
    return floor((data / m_Precision) + 0.5) * m_Precision;
}
...
void NormalEstimator::addValue(double data, double weight){
    ...
    if (m_SumOfWeights > 0) {
        ...
        double stdDev = sqrt( fabs(m_SumOfValuesSq - m_Mean * m_SumOfValues) / m_SumOfWeights);
        ...
        if (stdDev > 1e-10) {
            ...
            m_StandardDev = std::max( m_Precision / (2.0 * 3.0), stdDev);
        }
    }
}
...
double NormalEstimator::getMean(){
    return m_Mean;
}

double NormalEstimator::getStdDev(){
    return m_StandardDev;
}

double NormalEstimator::getPrecision(){
    return m_Precision;
}

double NormalEstimator::getSumOfWeights(){
    return m_SumOfWeights;
}

double NormalEstimator::normalProbability(double a){
...
double NormalEstimator::errorFunction(double x) { 
    double y, z;
    double T[] = {
    ...
    if( fabs(x) > 1.0 ) return( 1.0 - errorFunctionComplemented(x) );
    z = x * x;
    y = x * polevl( z, T, 4 ) / p1evl( z, U, 5 );
    return y;
}

double NormalEstimator::p1evl( double x, double coef[], int N ) {
...
double NormalEstimator::polevl( double x, double coef[], int N ) {
...
double NormalEstimator::errorFunctionComplemented(double a) { 
    double x,y,z,p,q;
