...
GPContainer* GPPopulation::evolution (GPPopulationRange& range, vector<int>& parents)
{
        ...
        // reproduction.
    {
        ...
        if (GPRandomPercent (GPVar.CrossoverProbability))
        {
            ...
            // (or whole population)
            gpCont=selectParents (range, parents);
...
void GPPopulation::generate (ostream& os, GPPopulation& newPop)
{
    ...
    if (GPVar.MaximumDepthForCrossover<GPVar.MaximumDepthForCreation)
        GPExitSystem ((char*)"GPPopulation::generate", 
                (char*)"MaximumDepthForCrossover is smaller than "\
    ...
    if (GPVar.DemeticGrouping)
    {
        ...
        if (demeSize<2)
            GPExitSystem ((char*)"GPPopulation::generate", 
                    (char*) "Demetic group size is smaller than 2");
        if (demeSize>containerSize ())
            GPExitSystem ((char*)"GPPopulation::generate", (char*)"Deme size too big");
        if (containerSize ()%demeSize!=0)
            GPExitSystem ((char*)"GPPopulation::generate", (char*)"Wrong deme size: "\
                    "doesn't fit");
    }
    else
    ...
    if (!GPVar.SteadyState)
    {
        ...
        if (GPVar.AddBestToNewPopulation){
            newPop.put (bestOfPopulation, NthGP (bestOfPopulation)->duplicate());
        ...
        for (int n=0; n<demeSize; )
        {
            ...
            for (int j=0; j<gpCont->containerSize (); j++)
            {
                ...
                if (n<demeSize)
                {
                    ...
                    // least we hpe so, otherwise an infinite loop
...
#if GPINTERNALCHECK
      if (!current)
	GPExitSystem ((char*) "GPPopulation::calculateStatistics", (char*)"GP is NULL");
#endif
...
void GPPopulation::printBrunoro(ostream& os, int gen){

    os<<"generation "<<gen<<":"<<endl;
    for(int i = 0; i < containerSize(); i++){
        GP* gp = NthGP(i);
        os<<"\tind: "<<"\t"<<100.0-gp->getFitness()<<"\t";
        gp->NthGene(0)->printBrunoro(os);
        os<<endl;
    }
}
