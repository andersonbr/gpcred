...
char* GPNode::load (istream& is)
{
  ...
  do 
    {
      if (!is)
	return (char*) "Character '\"' not found in stream";
      is >> line[0];
    }
  while (line[0]!='"');
  int ix=0;
  do 
    {
      if (!is)
	return (char*) "Matching character '\"' not found in stream";
      is >> line[ix++];
      if (ix>=(int)sizeof(line)) 
	return (char*) "Internal buffer overflow: matching character '\"' not found";
    }
  while (line[ix-1]!='"');
  line[ix-1]='\0';
...
{
  GPExitSystem((char*)"GPNodeSet::put", 
		(char*)"User must not use this function, but putNode() instead."); 
}
...
void GPNodeSet::putNode (GPNode& gpo)
{
  ...
  if (numFunctions+numTerminals==containerSize())
    GPExitSystem((char*)"GPNodeSet::putNode", (char*)"Node set is full"); 

  // Check if node with same identification number is already in set
  // (that's forbidden!)
  if (searchForNode (gpo.value()))
    GPExitSystem((char*)"GPNodeSet::putNode", 
		  (char*)"Node with same ID is not allowed in one node set");
#endif
...
#if GPINTERNALCHECK
  GPExitSystem((char*)(char*)"GPNodeSet::chooseNodeWithArgs", (char*)"Internal error"); 
#endif
...
#if GPINTERNALCHECK
  if (numFunctions==0)
    GPExitSystem((char*)(char*)"GPNodeSet::chooseFunction", 
		  (char*) "No function to choose from"); 
#endif
...
#if GPINTERNALCHECK
  if (numTerminals==0)
    GPExitSystem((char*)(char*)"GPNodeSet::chooseTerminal", 
		 (char*) "No terminal to choose from"); 
#endif
...
void GPAdfNodeSet::printBrunoro(ostream& os){

   //estou usando somente o nodeset 0:
   GPNodeSet* nds = NthNodeSet(0);
   nds-> printBrunoro(os);
    
}

void GPNodeSet::printBrunoro(ostream& os){
    ...
    for (int i=0; i<containerSize(); i++)
    {
        ...
        if (current)
        {
            if (current->isFunction() && current->arguments() == 2)
                operators.push_back(current->getRepresentation());
            else
                terms.push_back(current->getRepresentation());
        }
    }
    os<<"definition:"<<endl;
    os<<"\tops: ";
