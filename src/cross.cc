...
#if GPINTERNALCHECK
  if (totalLength==0)
    GPExitSystem ((char*)"GPGene::choose", 
		  (char*) "Tree length is 0: Can't select crossover point");
#endif
...
#if GPINTERNALCHECK
      if (!pg)
	GPExitSystem ((char*) "GPGene::choose",(char*) "Didn't find tree node");
#endif
			...
			int maxdepthforcrossover)
{
  ...
  if (parents->containerSize()!=2)
    GPExitSystem ((char*)"GP::cross", (char*)"Only two parents allowed for crossover");
#endif
  ...
  if (dad.containerSize()!=mum.containerSize())
    GPExitSystem ((char*)"GP::cross", 
		  (char*) "Mum and Dad must have same number of trees");
  if (dad.containerSize()==0)
    GPExitSystem ((char*)"GP::cross", (char*)"Parents contain no trees");
#endif
  ...
  if (!*rootGene1 || !*rootGene2)
    GPExitSystem ((char*)"GP::cross", 
		  (char*)"Genetic tree of Mum or Dad is NULL");
#endif
