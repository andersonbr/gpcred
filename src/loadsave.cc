...
#define MAXIMUMCLASSNUM 100   // Hope it won't be more. Otherwise change it
...
#if GPINTERNALCHECK
  GPExitSystem ((char*)"GPCreateRegisteredClassObject", 
		(char*)"Can't create object of class not registered");
#endif
  return NULL;
}
...
void GPRegisterClass (GPObject* gpo)
{
  ...
  if (registered==MAXIMUMCLASSNUM)
    GPExitSystem ((char*)"GPRegisterClass", 
		  (char*)"No more classes allowed. Increase MAXIMUMCLASSNUM");
#endif
