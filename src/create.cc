...
void GPPopulation::create ()
{
  ...
  if (adfNs->containerSize()<1)
    GPExitSystem ((char*) "Population::create", 
		  (char*) "ADF Node set contains no node sets");
  for (int tree=0; tree<adfNs->containerSize(); tree++)
    {
      GPNodeSet* ns=adfNs->NthNodeSet (tree);
      if (!ns)
	GPExitSystem ((char*)"Population::create", (char*)"Node set not defined");
      if (ns->containerSize()==0)
	GPExitSystem ((char*)"Population::create", (char*)"Node set contains no nodes");
      for (int i=0; i<ns->containerSize(); i++)
	if (!ns->Nth (i))
	  GPExitSystem ((char*)"Population::create",(char*) "Node in set not defined");
    }
#endif
  ...
  for (int i=0; i<containerSize (); i++)
      ...
      do
	{
	  ...
	  switch (GPVar.CreationType)
	    ...
	    default:
#if GPINTERNALCHECK
	      GPExitSystem ((char*)"Population::create",(char*) "Wrong creation type");
#endif
	      break;
	    }
		 ...
		 GPAdfNodeSet& adfNs)
{
  ...
  if (ctype!=GPGrow && ctype!=GPVariable)
    GPExitSystem ((char*)"GP::create",
		  (char*)"Argument ctype must be GPGrow or GPVariable");
#endif
