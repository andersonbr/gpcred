...
#if GPINTERNALCHECK
  if (container)
    GPExitSystem((char*)"GPContainer::reserveSpace", (char*)"Container not empty");
#endif
...
#if GPINTERNALCHECK
  if (!container)
    GPExitSystem((char*)"GPContainer::Nth", (char*)"No container");
  if (n>=contSize || n<0){
    std::cerr<<"numero de elementos => "<<contSize<< " -- " << " tentando acessar elemento: " << n << endl; 
    GPExitSystem((char*)"GPContainer::Nth", (char*)"Wrong range for index n");
  }
#endif
...
#if GPINTERNALCHECK
  if (!container)
    GPExitSystem((char*)"GPContainer::getPointerAdress", (char*)"No container");
  if (n>=contSize || n<0)
    GPExitSystem((char*)"GPContainer::getPointerAdress",  (char*)
...
#if GPINTERNALCHECK
  if (!container)
    GPExitSystem((char*)"GPContainer::put", (char*)"No container");
  if (n>=contSize || n<0)
    GPExitSystem((char*)"GPContainer::put", (char*)"Wrong range for index n");
#endif
...
#if GPINTERNALCHECK
  if (!container)
    GPExitSystem((char*)"GPContainer::get", (char*)"No container");
  if (n>=contSize || n<0)
    GPExitSystem((char*)"GPContainer::get", (char*)"Wrong range for index n");
  if (container[n]==NULL)
    GPExitSystem((char*)"GPContainer::get", (char*)"No object at position");
#endif
...
char* GPContainer::load (istream& is)
{
  ...
  is >> id;
  if (id!=isA ())
    return (char*)"Object ID GPContainerID expected, but not found";
