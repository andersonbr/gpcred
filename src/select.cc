					...
					GPPopulationRange& range)
{
  ...
  if (numToSelect!=1 && numToSelect!=2)
    GPExitSystem ((char*)"GPPopulation::tournamentSelection", 
		  (char*)"Wrong argument numToSelect");
  if (GPVar.TournamentSize<2)
    GPExitSystem ((char*) "GPPopulation::tournamentSelection", 
		  (char*)"Wrong tournament size");
  if (range.endIx-range.startIx<1)
    GPExitSystem ((char*) "GPPopulation::tournamentSelection", 
		  (char*) "Range to select from is empty");
#endif
...
#if GPINTERNALCHECK
  if (numToSelect<1)
    GPExitSystem ((char*)"GPPopulation::probablisticSelection",
		  (char*)"Wrong argument numToSelect");
#endif
				  ...
				  GPPopulationRange& range)
{
  ...
  switch (GPVar.SelectionType)
    ...
    default:
#if GPINTERNALCHECK
      GPExitSystem ((char*)"GPPopulation::select", (char*) "Unknown selection method");
#endif
      break;
    }
}
...
GPContainer* GPPopulation::selectParents (GPPopulationRange& range, std::vector<int>& parents)
{
  return 
    select (2, range, parents);
}
