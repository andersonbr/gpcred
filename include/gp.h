...
public:
  ...
  GPContainer& operator = (GPContainer& gpo) {
    GPExitSystem ((char*)"operator =", (char*)"Assignment operator not yet implemented"); 
    return gpo; }
...
public:
  GPGene () { node=0; }
  GPGene (GPNode& gpo)
    : GPContainer(gpo.arguments()),  node(&gpo){}
  ...
  char* getRepresentation() { return node->getRepresentation(); }
...
public:
  ...
  virtual GPContainer* select (int numToSelect, GPPopulationRange& range, std::vector<int>&);
  virtual GPContainer* selectParents (GPPopulationRange& range, std::vector<int>&);
  virtual void calculateStatistics ();
  virtual void evaluate();

  virtual void createGenerationReport (int printLegend, int generation,
				       ostream& fout, ostream& bout);
  GPContainer* evolution (GPPopulationRange& range, vector<int>&);
  virtual void generate (ostream&, GPPopulation& newPop);
  ...
  void printBrunoro(ostream&, int, vector<int>);
  void printBrunoro(ostream&, int);
