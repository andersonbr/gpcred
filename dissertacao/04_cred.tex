...
%O índice de GINI melhorado para classificação de atributos foi enunciado pela primeira vez no trabalho de~\cite{Shang07}. Ao contrario da maioria das métricas expostas nessa seção, o Índice de Gini melhorado tem apenas um parâmetro, o valor do $i$-ésimo atributo, não levando em consideração nenhuma classe específica. Ele é considerado melhorado por algumas pequenas diferenças com o método tradicional de GINI, amplamente utilizado nas ciências econômicas para avaliação de distribuição de renda pela população de um certo país ou região. A fórmula sugerida por \cite{Shang07} para ser utilizada em classificação de atributos textuais é dada por:
...
A razão de chances tem sido utilizada para selecionamento de atributos por \cite{dunja98} fazendo com que A seja $P(x_i|c_j)$ e B seja $P(x_i,\overline{c_j})$. Logo,
\begin{equation}\label{eqn::or}
