...
%\footnote{A palavra exemplo provém do vocabulário de mineração de dados. Um exemplo também pode ser chamado de instância ou de objeto presente em uma base de dados.}
...
% dado que elas provêm indícios de separações entre as classes.
%sendo que ele apresenta um conjunto de atributos que contribuem (positiva ou negativamente) para dizermos que o exemplo de teste pertence a determinada classe.
...
%Chamamos de relacionamentos as características que conseguimos utilizar para definir uma relação entre os exemplos. 
...
%temos então que um exemplo pode contribuir mais ou menos para classificar um outro quando levamos em consideração uma relação que envolva aqueles exemplos. 
...
%Como já se pode notar, em alguns cenários e problemas reais, não é tão simples modelar um relacionamento, ou até mesmo podemos não ter um relacionamento....falar do GP tb!
...
%Exploramos credi atributos....credi de graphs....gp....morreu bahia!

%Usualmente, os classificadores utilizam apenas os atributos dos exemplos de treinamento para construção de um modelo de classificação, além de assumir que todos os atributos e todos os exemplos têm a mesma significância.
%Entretanto, nem sempre esse é o caso, pois alguns atributos e até mesmo alguns exemplos podem ser mais úteis que outros.

%Acreditamos que as informações de que o valor de um atributo do exemplo de teste está atrelado a uma determinada classe ou de que o exemplo de teste tem algum tipo de relação forte com os exemplos de treinamento de certa classe,
%podem ser usadas para criação de melhores algoritmos de classificação. 
%Porém, temos que avaliar a credibilidade dessas informações, ou seja, definir a credibilidade de que um exemplo de teste pertença a uma classe, dados os seus atributos e os seus relacionamentos.


%Na literatura, a credibilidade é dada como uma característica dependente do observador. Ou seja, observadores diferentes podem atribuir valores diferentes de credibilidade para uma mesma informação. O cálculo da credibilidade varia de acordo com os fatores relevantes para o problema no qual ela é empregada.
%Em nosso caso, tratamos a credibilidade do ponto de vista de um classificador, que usa os conhecimentos a respeito dos atributos e relacionamentos existentes entre os exemplos para decidir se a credibilidade da informação de pertinência de um exemplo a uma classe é elevada ou não.

%Utilizamos métricas de seleção de atributos e métricas de redes complexas para nos auxiliar no cálculo da credibilidade baseada em atributos e em relacionamentos, respectivamente.
%Devido ao grande número de métricas cabíveis para cada caso, empregamos Programação Genética para a criação de funções de credibilidade 
%baseadas em atributos e relacionamentos.
%que estimam a credibilidade da informação que o teste pertence a uma determinada classe, dados os seus atributos e seus relacionamentos. 
%Essas funções foram incorporadas nos algoritmos de classificação Naïve Bayes e KNN, auxiliando-os a criarem modelos mais eficazes. 
%(valores)
...
%\item Definir os principais fatores que impactam no cálculo da credibilidade do ponto de vista de um classificador em alguns cenários específicos.
