...
Nessa seção apresentamos várias métricas utilizadas como terminais na construção dos indivíduos usados pela \textsc{PG}. 
...
Em suma, algumas métricas estão muito atreladas à classificação de documentos, em especial aquelas que realizam cálculos baseados na frequência de termos e documentos. 
...
As métricas usadas para atributos textuais consistem em algumas variantes do \textsc{TFIDF} (\cite{Salton88}), métrica mais difundida entre as apresentadas.

\begin{description}

%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Frequência do Termo}} %(TF)}
\label{item::sumtf}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Frequência do Termo por Classe}} % (\textsc{TFClasse})}
\label{item::tf}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Frequência dos Documentos por Termo}}% (DF)}
\label{item::df}
...
\item{\textbf{Frequência de Documentos por Termo-Classe}}% (DFClasse)}
\label{item::sumdf}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Inverso da Frequência de Documentos}}% (\textsc{IDF})}
\label{item::idf}
...
\item{\textbf{Inverso da Frequência de Documentos por Classe}}% (\textsc{IDFClasse})}
\label{item::idf}
...
\item{\textbf{Frequência do Termo Inverso da Frequência de Documentos}}% (\textsc{TFIDF})}
\label{item::tfidf}
...
\item{\textbf{Máxima Frequência do Termo Inverso da Frequência de Documentos}}% (\textsc{maxTFIDF})}
\label{item::maxtfidf}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Frequência do Termo Inverso da Frequência da Classe}}% (TFICF)}
\label{item::tficf}
...
\item{\textbf{Máxima Frequência do Termo Inverso da Frequência da Classe}}% (\textsc{maxTFICF})}
\label{item::maxtfidf}
...
Na Tabela~\ref{table::metricas_attr} está a referência quanto às variáveis usadas nessa seção.
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Medida de Ambiguidade}} % (AM)}
\label{item::am}

A medida de ambiguidade (\textsc{AM} de \textit{Ambiguity Measure}) foi definida por~\cite{Mengle08} e utilizada como um método de seleção de atributos. Ela atribui valores maiores para os atributos considerados menos ambíguos. Assim, ela considera que um atributo não é ambíguo quando sua presença indica, com um alto grau de confiança, que o exemplo de teste pertence a uma classe específica. Podemos calcular $AM(x_i, c_j)$ como:
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Probabilidade Condicional}} % ($P(x_i|c_j)$)}
\label{item::pc}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Inverso da Probabilidade Condicional}}% ($P(x_i|c_j)$)}
\label{item::pc'}
...
%%%%%%%%%%%%%%%%%%%-------------------------------------------------------%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item{\textbf{Índice de Gini Melhorado}} % (GINI)}
\label{item::gini}
...
O Coeficiente de Correlação (do inglês, \textit{Correlation Coefficient}, \textsc{CC}) é uma métrica de seleção de atributos variante da métrica \textit{CHI-Quadrada}. Definida por~\cite{Ng97}, temos que $(CC)^2 = \chi^2$, logo:
\begin{equation}\label{eqn::ce}
